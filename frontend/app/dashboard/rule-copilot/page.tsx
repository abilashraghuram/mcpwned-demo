...
import { RuleCopilotDiagramCache, PlaygroundDiagram } from "@/utils/cache";
import * as EmailValidator from "email-validator";
import { Label } from "@/components/ui/label";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import RuleCopilotGuardrailsList from "./components/RuleCopilotGuardrailsList";
...
export default function PlaygroundPage() {
  ...
  useEffect(() => {
    const cached = RuleCopilotDiagramCache.load();
    if (cached && Array.isArray(cached.diagrams) && cached.diagrams.length > 0) {
      ...
      const diagramsTyped = cached.diagrams.map((d: PlaygroundDiagram) => ({
        ...d,
        nodes: d.nodes as XYNode[],
        edges: d.edges as XYEdge[],
        guardrails: d.guardrails || d.guardrail_justification || [],
      }));
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const toolsRes = await fetch("/api/mcp-tools-rule-page", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ github_readme: readmeUrl.trim(), guid, email: email.trim(), user_exploit_summary: userQuery }),
      });
      ...
      const pollForResult = async () => {
        ...
        try {
          const statusRes = await fetch(`/api/rule-diagram/status?guid=${guid}`);
          const statusData = await statusRes.json();
          if (statusRes.status === 200 && Array.isArray(statusData.diagrams)) {
            const diagramsWithGuardrails = statusData.diagrams.map((d: PlaygroundDiagram) => ({
              ...d,
              guardrails: d.guardrails || d.guardrail_justification || [],
            }));
            setDiagrams(diagramsWithGuardrails);
            setSelectedDiagramIdx(0);
            setLoading(false);
            clearInterval(pollInterval);
            RuleCopilotDiagramCache.save(diagramsWithGuardrails, email.trim(), readmeUrl.trim());
            RuleCopilotDiagramCache.saveByScanDescription(diagramsWithGuardrails);
  ...
  return (
    <DashboardShell>
      ...
      <DashboardHeader
        heading="Rule Copilot"
        text="Generate guardrails for any MCP server"
      />
        ...
        <main className="flex-1 px-0 md:px-4">
          ...
          {loading && (
            <div className="flex items-center justify-center my-8">
              ...
              <p className="text-2xl text-primary">Generating guardrails...</p>
            </div>
          )}
          ...
          {selectedDiagram && (
            <div className="flex flex-col flex-1 min-h-[500px]">
              ...
              <div className="mt-8 flex flex-col md:flex-row gap-6 md:gap-0 md:space-x-6 items-stretch">
                <RuleCopilotGuardrailsList guardrails={selectedDiagram.guardrails} guardrail_code={selectedDiagram.guardrail_code} className="mx-auto w-full" />
              </div>
            </div>
          )}
        </main>
